"""Creates dataset based on strebelle training image."""
import math
import os
...
class DatasetCreator:
    ...
    def __init__(self, ti_image_path: str, crop_size: int = 64,
                 stride: int = 16) -> None:
        """Build constructor for DatasetCreator."""
        self.original_TI: List[numpy.uint8] = cv2.imread(ti_image_path)
        self.crop_size: int = crop_size
        self.stride: int = stride

    def complete_square_image(self, size: int = 64) -> Any:
        """Fill image with black stripes to complete the square image shape."""
        if self.original_TI.shape[0] < size:
            strip = numpy.zeros(
                (size - self.original_TI.shape[0],
                 self.original_TI.shape[1], 3), dtype=numpy.int8)
            img = numpy.vstack((self.original_TI, strip))

        if self.original_TI.shape[1] < size:
            strip = numpy.zeros(
                (self.original_TI.shape[0],
                 size - self.original_TI.shape[1], 3), dtype=numpy.int8)
            img = numpy.hstack((self.original_TI, strip))

        return img

    def yield_cropped_image(self) -> Tuple[int, int, Any]:
        """Generate crops for the input image."""
        ...
        for i in range(y_steps):
            for j in range(x_steps):
                ...
                if (cropped_image.shape[0] != self.crop_size or
                        cropped_image.shape[1] != self.crop_size):
                    cropped_image = self.complete_square_image(
                        cropped_image, self.crop_size)
