...
def create_dataset(file_path):
    print("Dataset filepath: ", file_path)
...
def prepare_for_training(ds, cache=True, shuffle_buffer_size=1000):
  ...
  # fit in memory.
  print('Dataset shape', len(list(ds)))
  if cache:
...
X_train = create_dataset(PATH+'/TI_augmented/data/*')
...
def train(iterations, batch_size, sample_interval):
    X_dataset = create_dataset(PATH+"/TI_augmented/data/*")
    X_train = prepare_for_training(X_dataset)
    print('Type and shape of X_train after prepared for training : ',type(X_dataset))
    ...
    for iteration in range(iterations):
        ...
        if (iteration + 1) % sample_interval == 0:
            ...
            print(f"{iteration} [D loss: {d_loss}, acc.: {accuracy}]")
...
def timeit(ds, steps=default_timeit_steps):
    ...
    duration = end-start
    print("{} batches: {} seconds".format(steps, duration))
