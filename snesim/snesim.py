...
def etype_plot(dict_ti: pd.DataFrame):
    ...
    ax.imshow(_data["etype"].values.reshape(150, 150), cmap="gray", origin="lower")
...
def plot_uncertainty(dict_ti: pd.DataFrame):
    ...
    plt.imshow(_data["u_min"].values.reshape(150, 150), cmap="gray", origin="lower")
...
def plot_realizations_grid(real):
    ...
    image_grid = ImageGrid(
        fig, 111, nrows_ncols=(3, 3), axes_pad=0.1, cbar_location="left", cbar_pad=0.15
    )
...
def proportions(real: numpy.ndarray) -> None:
    ...
    # Boxplot
    bp = ax.boxplot(
        df, labels=["Sandstone", "Shale"], patch_artist=True, showfliers=False
    )
...
def plot_ti_with_samples(ti: numpy.ndarray) -> None:
    fig, ax = plt.subplots(1, 2, figsize=(6, 8), constrained_layout=True)
